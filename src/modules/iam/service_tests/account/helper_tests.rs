//! iam AccountService 助手方法 + 边界单测（10 例 = 3+2+5）
//!
//! 覆盖 `AccountService::menus_for_roles` / `current_user_out` 助手 + 边界用例
...
//! `menus_for_roles` / `current_user_out` 不再要求 `&mut dyn IamUnitOfWork`。
...
async fn current_user_out_returns_user_with_menus() {
    ...
    let out: CurrentUserOut = svc
        .current_user_out(&mut repo, &current)
        .await
        .unwrap();
...
async fn current_user_out_returns_not_found_when_user_missing() {
    ...
    let res = svc
        .current_user_out(&mut repo, &manager_current())
        .await;
...
async fn create_user_trims_and_lowercases_username_and_full_name() {
    ...
    svc.create_user(&mut repo, &req, &manager_current())
        .await
        .unwrap();
