...
// 2026-09-15 followup-cleanup A8：`CncProgramService` 与 `PartFileService` 已迁出到
// `src/modules/part_file/handler.rs`（part 维度文件路由集中处）。
...
// 2026-09-15 followup-cleanup A8：原 `// ===== 2026-09-15 takeover-fill：part 文件路由（Phase 3 补齐） =====`
// 段（含 read_part_file_multipart + 7 个文件 handler）已迁出到
// `src/modules/part_file/handler.rs::part_nested_router()`。本文件由 1491 行 → 1248 行，
// 重新符合 1000 行上限要求。`part::router()` 改为 nest 该子路由。